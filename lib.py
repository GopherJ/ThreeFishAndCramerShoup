...
def readFile(filename, blockSize):
    ...
    for m in range(len(newArr)):
        newArr[m] = ''.join([padRight(bin(c).replace('0b', ''), '0', 8) for c in newArr[m]])
    
    # return arr of decimal
    return [int('0b' + el, 2) for el in newArr]

def writeFile(filename, arr):
    fo = open(filename, 'w')
    newArr = dechunk([chunk(padRight(bin(el).replace('0b', ''), '0', 64), 8) for el in arr])
    ...
    fo.write(bytesToUtf8(bytes([int('0b' + el, 2) for el in newArr])))
    return fo.close()
