...
LE32 = lambda str: ("".join(chunk(str, 8)[::-1]))
LE64 = lambda str: ("".join(dechunk([LE32(i) for i in chunk(str, 32)][::-1])))
LE   = lambda hex: ("".join(chunk(hex.replace("0x", ""), 2)[::-1]))
...
def mainLoop(m):
    ...
    for i in range(64):
        ...
        d = c
        c = b
        b = (b + L(a + f + k[i] + int(LE32(m[g]), 2), s[i])) & 0xffffffff
        a = tmp
...
def md5(s):
    ...
    return "".join([LE(hex(i)) for i in [A, B, C, D]])
